//
//  UIView+LTGestureCallbackValues.m
//  LTCatrgory
//
...
const NSString *UIView_LTGestureCallback_gesturesKey = @"UIView_LTGestureCallback_gesturesKey";
const NSString *UIView_LTGestureCallback_gestureKeysHashKey = @"UIView_LTGestureCallback_gestureKeysHashKey";

@implementation LTGestureCallbackValues
...
@implementation UIView (LTGestureCallbackValues)
...
-(void)addTapGestureRecognizer:(void(^)(UITapGestureRecognizer* recognizer, NSString* gestureId))tapCallback  tapGestureId:(NSString*)tapGestureId  numberOfTapsRequired:(NSUInteger)numberOfTapsRequired  numberOfTouchesRequired:(NSUInteger)numberOfTouchesRequired
{
    ...
    LTGestureCallbackValues *v = [LTGestureCallbackValues new];
    v.gesture = tg;
...
{
    LTGestureCallbackValues *v = [self.gestures objectForKey:tapGestureId];
    if (v != nil) {
...
-(void)removeAllTapGestures
{
    ...
    for (LTGestureCallbackValues *v in arr) {
...
{
    LTGestureCallbackValues *v = [self.gestureKeysHash objectForKey:[NSString stringWithFormat:@"%lu", (unsigned long)recognizer.hash]];
...
{
    LTGestureCallbackValues *v = [self.gestures objectForKey:pinchGestureId];
    if (v != nil) {
...
{
    LTGestureCallbackValues *v = [self.gestures objectForKey:panGestureId];
    if (v != nil) {
...
{
    LTGestureCallbackValues *v = [self.gestures objectForKey:swipeGestureId];
    if (v != nil) {
...
{
    LTGestureCallbackValues *v = [self.gestures objectForKey:rotationGestureId];
    if (v != nil) {
...
{
    LTGestureCallbackValues *v = [self.gestures objectForKey:longPressGestureId];
    if (v != nil) {
...
-(NSMutableDictionary *)gestures {
    NSMutableDictionary *dict = objc_getAssociatedObject(self, &UIView_LTGestureCallback_gesturesKey);
...
{
    objc_setAssociatedObject(self, &UIView_LTGestureCallback_gesturesKey, value, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

-(NSMutableDictionary *)gestureKeysHash {
    NSMutableDictionary *dict = objc_getAssociatedObject(self, &UIView_LTGestureCallback_gestureKeysHashKey);
...
{
    objc_setAssociatedObject(self, &UIView_LTGestureCallback_gestureKeysHashKey, value, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
